...
# Import my stuff
import losses
import third_party.BigGAN as model
from third_party import inception_tf
from third_party import utils
from third_party import train_fns
from third_party.augment import AugmentPipe
# The main training file. Config is a dictionary specifying the configuration
# of this training run.


def run(config, ema_losses):
            ...
            if config['pbar'] == 'mine':
                print(', '.join(['itr: %d' % state_dict['itr']]
                                + ['%s : %+4.3f' % (key, metrics[key])
                                    for key in metrics if 'ema' not in key]), end=' ')
