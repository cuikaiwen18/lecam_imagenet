...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None,
                 ...
                 index_filename='imagenet_imgs.npz',  seed=0,  num_samples=None, **kwargs):
        ...
        print('Sampling limited samples:', num_samples)
        if num_samples is not None:
            np.random.seed(seed)
            train_idx = np.random.choice(len(imgs), num_samples, replace=False)
            imgs = imgs[train_idx]
        print('Sampled limited samples:', len(imgs))
