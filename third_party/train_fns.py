...
from third_party.DiffAugment_pytorch import DiffAugment
    ...
    def train(x, y):
        ...
        for step_index in range(config['num_D_steps']):
            ...
            for accumulation_index in range(config['num_D_accumulations']):
                ...
                D_loss_real, D_loss_fake = losses.discriminator_loss(
                    D_fake, D_real, ema_losses, state_dict['itr'])
                ...
                D_loss = D_loss_real + D_loss_fake + D_loss_LC
        ...
        if not config['fix_G']:
            ...
            for accumulation_index in range(config['num_G_accumulations']):
                ...
                # Accumulated G_loss
                G_loss_total += (G_loss.item() / float(config['num_G_accumulations']))

            # Accumulation
            ema_losses.update(G_loss_total, 'G_loss', state_dict['itr'])            
        ...
        out = {'G_loss': float(G_loss_total) if not config['fix_G'] else 0.,
               'D_loss_real': float(D_loss_real_total),
               'D_loss_fake': float(D_loss_fake_total),
               'D_real': float(D_real_total),
               'D_fake': float(D_fake_total),
               }
        if config['LC'] > 0:
            out['D_loss_LC'] = float(D_loss_LC.item())
...
def test(G, D, GD, G_ema, z_, y_, state_dict, config, sample, get_inception_metrics,
    ...
    test_log.log(itr=int(state_dict['itr']), IS_mean=float(IS_mean),
                 ...
                 D_acc_val=D_acc_val, D_acc_train=D_acc_train,
